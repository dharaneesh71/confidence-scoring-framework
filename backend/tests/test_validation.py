...
def test_save_model_version_creates_file(tmp_path):
    from services.llama_service import LlamaService
    svc = LlamaService.__new__(LlamaService)
    svc.finetuned_path = tmp_path / "model"

    # Patch Path so history file goes to tmp_path
    history_file = tmp_path / "model_history.json"
    with patch("services.llama_service.Path") as mock_path:
        mock_path.return_value = history_file
        svc.save_model_version({"accuracy": 0.85, "f1": 0.82, "loss": 0.15})

    # Direct call without patch
    svc2 = LlamaService.__new__(LlamaService)
    svc2.finetuned_path = tmp_path / "model"

    import services.llama_service as lsvc
    original_path = lsvc.Path
    lsvc.Path = lambda x: tmp_path / x if "model_history" in str(x) else original_path(x)

    svc2.save_model_version({"accuracy": 0.85, "f1": 0.82, "loss": 0.15})
...
def test_hot_swap_restores_on_failure():
    ...
    assert svc.model is original_model   # restored
